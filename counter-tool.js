...
function normalizeCounterState(){
  ...
  counterState.counters=(Array.isArray(counterState.counters)?counterState.counters:[]).map(c=>({id:c.id||counterId(),name:String(c.name||'項目'),count:Math.max(0,Number(c.count)||0),type:c.type==='derived'?'derived':'manual',sources:Array.isArray(c.sources)?c.sources:[],color:typeof c.color==='string'?c.color:''}));
...
function counterRender(){
  ...
  counterState.counters.forEach(c=>{
    ...
    const colorStyle=c.color?` style="--counter-color:${counterEsc(c.color)}"`:' ';
    ...
    wrap.innerHTML=`<div class="counter-swipe-actions"><button type="button" class="counter-delete-btn">削除</button></div>
      <div class="counter-row ${derived?'counter-derived':''}"${colorStyle}>
        <button type="button" class="counter-drag-handle" aria-label="並べ替え" title="ドラッグして順番変更">☷</button>
        <button type="button" class="counter-color-btn" aria-label="色を変更" title="色を変更" style="--swatch:${counterEsc(c.color||'#6b7280')}"></button>
        ...
        ${derived?'':'<div class="counter-actions"><button type="button" class="counter-minus">−</button><button type="button" class="counter-plus">＋</button></div>'}
      </div>`;
    ...
    wrap.querySelector('.counter-color-btn').onclick=()=>counterOpenColorPicker(c.id);
...
function counterOpenColorPicker(id){
  const c=counterFind(id);if(!c)return;
  let pop=document.getElementById('counterColorPopover');
  if(!pop){pop=document.createElement('div');pop.id='counterColorPopover';pop.className='counter-color-popover';document.body.appendChild(pop)}
  const colors=['#ef4444','#f97316','#eab308','#22c55e','#14b8a6','#06b6d4','#3b82f6','#6366f1','#8b5cf6','#ec4899','#a855f7','#6b7280'];
  pop.innerHTML='<div class="counter-color-title">色を選択</div><div class="counter-color-grid">'+colors.map(col=>`<button type="button" class="counter-palette-btn" style="--swatch:${col}" data-color="${col}" aria-label="${col}"></button>`).join('')+'<button type="button" class="counter-palette-clear" data-color="">色なし</button></div>';
  pop.querySelectorAll('[data-color]').forEach(b=>b.onclick=()=>{c.color=b.dataset.color||'';counterRender();pop.remove()});
  document.body.appendChild(pop);
  const rect=document.querySelector(`.counter-wrap[data-id="${CSS.escape(id)}"] .counter-color-btn`)?.getBoundingClientRect();
  if(rect){pop.style.left=Math.min(window.innerWidth-pop.offsetWidth-8,Math.max(8,rect.left));pop.style.top=Math.min(window.innerHeight-pop.offsetHeight-8,rect.bottom+8)+'px'}
  setTimeout(()=>document.addEventListener('pointerdown',function close(e){if(!pop.contains(e.target)){pop.remove();document.removeEventListener('pointerdown',close)}},{once:true}),0);
}
function counterOpenAggregateDialog(editId=null){
  const current=editId?counterFind(editId):null;
  const list=counterSourceChoices(editId||'').map(x=>x.c);
  if(!list.length){alert('先に合算元の項目を追加してください');return}
  let dlg=document.getElementById('counterAggregateDialog');
  if(!dlg){dlg=document.createElement('div');dlg.id='counterAggregateDialog';dlg.className='counter-modal-backdrop';dlg.innerHTML='<div class="counter-modal" role="dialog" aria-modal="true"><h3>合算を設定</h3><label>合算名<input id="counterAggName" placeholder="例：CZ合算"></label><div class="counter-modal-sub">合算する項目を選択</div><div id="counterAggChoices" class="counter-agg-choices"></div><div class="counter-modal-actions"><button type="button" id="counterAggCancel">キャンセル</button><button type="button" class="primary" id="counterAggOk">保存</button></div></div>';document.body.appendChild(dlg);}
  dlg.querySelector('#counterAggName').value=current?.name||'合算';
  const selected=new Set(current?.sources||[]);
  dlg.querySelector('#counterAggChoices').innerHTML=list.map(c=>`<label class="counter-agg-option"><input type="checkbox" value="${c.id}" ${selected.has(c.id)?'checked':''}><span>${counterEsc(c.name)}</span></label>`).join('');
  dlg.hidden=false;
  dlg.querySelector('#counterAggCancel').onclick=()=>{dlg.hidden=true};
  dlg.querySelector('#counterAggOk').onclick=()=>{const name=dlg.querySelector('#counterAggName').value.trim();const ids=[...dlg.querySelectorAll('#counterAggChoices input:checked')].map(x=>x.value);if(!name){alert('合算名を入力してください');return}if(!ids.length){alert('合算元を1つ以上選択してください');return}if(current){current.name=name;current.sources=ids}else{counterState.counters.push({id:counterId(),name,count:0,type:'derived',sources:ids,color:''})}dlg.hidden=true;counterRender()};
}
...
function counterAdd(){const el=document.getElementById('counterNewName'),name=el?.value.trim();if(!name)return;counterState.counters.push({id:counterId(),name,count:0,type:'manual',sources:[],color:''});el.value='';counterRender()}
function counterAddAggregate(){counterOpenAggregateDialog()}
function counterEditAggregate(id){counterOpenAggregateDialog(id)}
